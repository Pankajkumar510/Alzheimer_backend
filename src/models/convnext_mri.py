...
def get_transforms(image_size: int = 224, is_train: bool = True):
    if is_train:
        return T.Compose([
            ...
            T.RandomHorizontalFlip(p=0.5),  # left-right symmetry is acceptable for axial brain
...
def create_mri_model(num_classes: int, model_name: str = "convnext_base", pretrained: bool = True, drop_rate: float = 0.0) -> nn.Module:
