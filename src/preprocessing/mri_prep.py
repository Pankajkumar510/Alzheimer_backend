...
def process_input(input_path: Path, output_dir: Path, label: str, strategy: str, image_size: int) -> List[Dict]:
    rows: List[Dict] = []
    if input_path.is_dir():
        # recurse
        for p in input_path.rglob("*"):
            if p.is_file() and (is_nifti_file(str(p)) or is_image_file(str(p))):
                rows.extend(process_input(p, output_dir, label, strategy, image_size))
    ...
    if is_image_file(str(input_path)):
        # treat as 2D slice; replicate to RGB
        import numpy as np
        import cv2
...
def main():
    ap = argparse.ArgumentParser(description="MRI preprocessing to 224x224 PNGs and CSV metadata")
    ap.add_argument("--input", required=True, help="Path to MRI input (file or directory)")
    ...
    ap.add_argument("--strategy", choices=["multislice", "mid"], default="multislice")
    ...
    rows.extend(process_input(input_path, output_dir, args.label, args.strategy, args.image_size))
    if not rows:
        print("No MRI inputs processed. Check paths and formats.")
        return
